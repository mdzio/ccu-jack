...
func (c *mqttTemperature) start() {
	c.temperatureSubscribedTopic = c.temperatureTopic.Value().(string)
	if c.temperatureSubscribedTopic != "" {
		...
		c.mqttServer.Subscribe(c.temperatureSubscribedTopic, message.QosExactlyOnce, &c.temperatureOnPublish)
	}

	c.humiditySubscribedTopic = c.humidityTopic.Value().(string)
	if c.humiditySubscribedTopic != "" {
		...
		c.mqttServer.Subscribe(c.humiditySubscribedTopic, message.QosExactlyOnce, &c.humidityOnPublish)
	}
}
